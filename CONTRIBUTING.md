...
The client talks to the Tajeer AI backend. Point it at yours:
...
device needs your LAN address. Without the define, the environment's default
applies — see `lib/app/config/app_config.dart`.

Select a deployment with `--dart-define=TAJEER_ENV=development|staging|production`.
